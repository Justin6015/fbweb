# Erste öffentliche Webseite

- <Badge type="info" text="Jahr: 2021 bis 2026" />
  <Badge type="tip" text="Privat" />

## Technologien

- IDE: VSC
- PHP
- HTML, CSS
- Internetinformationsdienste (IIS)-Manager

Screenshot von friedrichbeckmann.de root:
![Screenshot Website Root friedrichbeckmann.de](fbWeb1-root.png)

Screenshot von friedrichbeckmann.de/sanke Snake:  
Siehe auch: [Projekt Snake](../snake)
![Screenshot Website Snake friedrichbeckmann.de](fbWeb1-snake.png)

Screenshot von friedrichbeckmann.de/pw Passwort Generator:
![Screenshot Website Snake friedrichbeckmann.de](fbWeb1-pwGenerator.png)

## Minecraft Server Monitoring

Meine zweite Version von Minecraft Server Monitoring.  
Im Jahr <Badge type="info" text="2024" /> komplett neu geschrieben zur Vorbereitung für die Klausur "Einführung in die Webentwicklung".

- HTML, CSS
- PHP: Seitenklassen (Vererbungstechnik)

Screenshot von friedrichbeckmann.de/minecraft Live Monitoring von meinem Minecraft SMP Server:
![Screenshot Minecraft Server offline](./fbWeb1-minecraft-online.png)
![Screenshot Minecraft Server offline](./fbWeb1-minecraft-one-player-online.png)
![Screenshot Minecraft Server offline](./fbWeb1-minecraft-offline.png)
